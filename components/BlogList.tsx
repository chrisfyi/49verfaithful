...
function BlogList({posts}: Props) {
  return (
    <div>
        <hr className="border-[#B3995D] mb-10" />
        <div className="grid grid-cols-1 md:grid-cols-2 px-10 gap-10 gap-y-16 pb-24">
            ...
            <div key={post._id} className='flex flex-col group'>
                    ...
                    <div className="absolute bottom-0 w-full bg-black bg-opacity-20 backdrop-blur-lg rounded drop-shadow-lg text-white p-5 flex justify-between">
                        ...
                        <div className="flex flex-col md:flex-row gap-y-2 md:gap-x-2 items-center">
                            {post.categories.map(category => (
                                <div className="bg-[#AA0000] text-center text-white px-3 py-1 rounded-full text-sm font-semibold">
                                    <p>{category.title}</p>
                                </div>
                            ))}
                        </div>
                ...
                <p className="mt-5 font-bold flex items-center group-hover:underline">Read Post
                    <ArrowUpRightIcon className="mt-2 h-4 w-4" />
                </p>
            </div>
            </ClientSideRoute>
            ))}
